 ...
 *
 *
 * CJ: Latest Revision in Jan'18
 * - This code fits gradient b/w each data-pair. 
 * - All pairs within a run are considered
 * - Simple chi^2 is used as cost function (Mullet et al., 2015 -> MKK15)
 * - Parallelized
 * - New flowlaws added: 3 eqns for Peierls & GBS (all dry)
 *   a 4th eqn for Peierls & wet GBS
 * - Added a new option to regroup data pairs when inter-run bias=0 in a study
 *   ( to save time. 
 *   Default: CG search over ALL possible pairs; takes v long.
...
int main(int argc, char **argv)
{
    ...
    if (id==0)
    cerr << "Total no. of processors: " << proc << '\n';
    ...
    long idum = long(-(abs(ran_seed+id)+1)); // for ran2()
    model.setUp();
    if (id==0){
	...
	if (ndata!=nNewRuns)
	  error("Size of input new runID and rada must be the same\n");
      }
    }
    ...
    Array1d<double> chi2_vec_tot(max_m*2);
    Array1d<double> calcUnfixedParam; 
    Array1d<double> calcUnfixedParam_tot; 
    double chi2 = 0;
    double chi2_orig = 0;
    double chi2min = 1e30;
    Array1d<double> prev_minunfixParam; 
    Array1d<double> bcastData;
    Array1d<int> igrp;
    int gi;
	...
	if (ii%dn_ran==0){
	    ...
	    bcastData.resize(ndata*col);
	    if (id==0){
	       bcastData = model.outData();
	       if (outRanData)
	...
	// The data randomized by processor id=0 is broadcasted to all other processors
	// Also, the parameter chosen for randomization is b'casted to all proc.
	if (ii%dn_ran==0){
	  bcastData.push_back(1);
	  bcastData(ndata*col+1) = k;
	  MPI_Bcast(bcastData.begin(),col*ndata+1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	  
	  k=bcastData(ndata*col+1);

	  // Now, all proc have the same randomized data and the same 
	  // chosen parameter (k). 
	  // We will need to reset isSticky and pp to ensure that all 
	  // proc are working
	  // with the identical values of these flags and counters.
	  isSticky = false;
	  pp = 1;
	  if (sticky){
	    int istart, iend;
	  ...
	  // if (ii%dn_ran!=0) -> then only Bcast k.
	    MPI_Bcast(&k,1,MPI_INT,0,MPI_COMM_WORLD);

	  // Now, all proc have the same chosen parameter (k). 
	  // We will need to reset isSticky and pp to ensure that all 
	  // proc are working
	  // with the identical values of these flags and counters.
	    isSticky = false;
	...
	/*	
	// Now, all proc have the same randomized data and the same chosen parameter (k)
	// We will need to reset isSticky and pp to ensure that all proc are working
	// with the identical values of these flags and counters.
	isSticky = false;
	if (sticky){
	  for (int iig=1; iig<=gi; iig++){
	...
	*/
	...
	calcUnfixedParam_tot.resize(max_m*(scalecount+nbias+pp));

	calcUnfixedParam = 0;
        calcUnfixedParam_tot = 0;
	chi2_vec = 0;
	chi2_vec_tot = 0;
	chi2 = 0;
	chi2_orig = 1e30;

	for (int m=id; m<max_m; m+=proc){
		...
		// flow law willl need to be re-evaluated. callBeforBestFitA() does that.
	     ...
	     chi2 = model.calcChiSq3(k,chi2_orig);
	     chi2_vec(2*m+1) = chi2;
	     chi2_vec(2*m+2) = chi2_orig;
	     if (!isSticky)
	       calcUnfixedParam((nbias+scalecount+1)*m+1) = model.unfixedParam(k)->value();
	     else{
	       for (int iig=1; iig<=pp; iig++){
		 calcUnfixedParam((nbias+scalecount+pp)*m+iig) = model.unfixedParam(igrp(iig))->value();
	       }
	     }
	       	       
	     int p=1;
	     for (p=1; p<=nbias; p++){
	       calcUnfixedParam((nbias+scalecount+pp)*m+pp+p) = model.unfixedParam(model.numUnfixedParams()-nbias+p)->value();
	     }
	     for (int q=1; q<=model.numParallel(); q++){
	       if (model.parallel(q)->needScaling()){
		 calcUnfixedParam((nbias+scalecount+pp)*m+pp+p) = model.parallel(q)->scaling();
		 p++;
	       }
	     }
	     for (int q=1; q<=model.numSequential(); q++){
	       if (model.sequential(q)->needScaling()){
		 calcUnfixedParam((nbias+scalecount+pp)*m+pp+p) = model.sequential(q)->scaling();
		 p++;
	       }
	     }
	}

	MPI_Reduce (chi2_vec.begin(), chi2_vec_tot.begin(), 2*max_m, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD); 
	MPI_Reduce (calcUnfixedParam.begin(), calcUnfixedParam_tot.begin(), (nbias+scalecount+pp)*max_m, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	...
	if (id==0){
	  ...
	  for (int m=1; m<=max_m; m++){ 
	    if (chi2_vec_tot(2*m-1) <= chi2min){
	       chi2min = chi2_vec_tot(2*m-1);
	       m_chi2min = m;
	    }
	  }
	  if (m_chi2min>0){
	    for (int p=1; p<=(nbias+scalecount+pp); p++)
	      sampled_mval_m(p) = calcUnfixedParam_tot((nbias+scalecount+pp)*(m_chi2min-1)+p); 
	  }//else{
	    //cerr << "prev. min. chi2=" << chi2min << " was not exceeded.\n";
	  //}
	  ...
	  while (true){
	    ...
	    double prob = exp(-0.5*(chi2_vec_tot(2*m-1)-chi2min));
	    if (ran2(&idum)<prob){
	          chi2 = chi2_vec_tot(2*m-1);
		  chi2_orig = chi2_vec_tot(2*m);
		  if (!isSticky)
		    model.unfixedParam(k)->setValue(calcUnfixedParam_tot((nbias+scalecount+1)*(m-1)+1));
		  else{
		    for (int iig=1; iig<=pp; iig++)
		      model.unfixedParam(igrp(iig))->setValue(calcUnfixedParam_tot((nbias+scalecount+pp)*(m-1)+iig));
		  }
		  int p=1;
		  for (p=1; p<=nbias; p++){
		    model.unfixedParam(model.numUnfixedParams()-nbias+p)->setValue(calcUnfixedParam_tot((nbias+scalecount+pp)*(m-1)+pp+p));
		  }
		  for (int q=1; q<=model.numParallel(); q++){
		    if (model.parallel(q)->needScaling()){
		      model.parallel(q)->setScaling(calcUnfixedParam_tot((nbias+scalecount+pp)*(m-1)+pp+p));
		      p++;
		    }
		  }
		  for (int q=1; q<=model.numSequential(); q++){
		    if (model.sequential(q)->needScaling()){
		      model.sequential(q)->setScaling(calcUnfixedParam_tot((nbias+scalecount+pp)*(m-1)+pp+p));
		      p++;
		    }
		  }	     
		  break;
		...
		chi2 = chi2_vec_tot(2*m_chi2min-1);
		chi2_orig = chi2_vec_tot(2*m_chi2min);
		if (!isSticky)
	...
	MPI_Bcast(setParams.begin(), model.numUnfixedParams()+scalecount, MPI_DOUBLE, 0 , MPI_COMM_WORLD);
	for (p=1; p<=model.numUnfixedParams(); p++){
	...
	if (id==0){
	  ...
	  if (ii%dn_out == 0){
		...
		//		cerr << model.unfixedParam(i)->name() <<" ";
		cout << line;
	      }
